...
const ST_HomeScreen = () => {
  const [studentName, setStudentName] = useState(''); // State to hold student name
  ...
  useEffect(() => {
    ...
    const fetchStudentName = async () => {
      try {
        ...
        const userDoc = await firebase.firestore().collection('students').doc(userId).get(); // Fetch user document
        if (userDoc.exists) {
          const userData = userDoc.data(); // Get user data
          const { firstName, lastName } = userData; // Extract first name and last name
          setStudentName(`${firstName} ${lastName}`); // Set student name state
        } else {
    ...
    fetchStudentName(); // Call the fetch function
  }, []); // Empty dependency array ensures the effect runs only once after initial render

  return (
    <View style={styles.container}>
      <View style={styles.welcomeContainer}>
        <Text style={styles.welcomeMessage}>Welcome, {studentName}!</Text>
      </View>
